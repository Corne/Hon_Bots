...
--Stored References
--TODO: add IsValid() checks for all of these 
core.enemyBuildings = {}
core.enemyTowers = {}
core.enemyRax = {}
...
core.enemyOtherBuildings = {}
core.allyBuildings = {}
core.allyTowers = {}
core.allyRax = {}
core.allyMainBaseStructure = nil
core.allyWell = nil
core.allyWellAttacker = nil
core.allyOtherBuildings = {}
core.shops = {}
...
function core.CoreInitialize(controller)
	...
	core.enemyBuildings			= sortedBuildings.enemyBuildings
	...
	core.enemyOtherBuildings	= sortedBuildings.enemyOtherBuildings
	core.allyBuildings			= sortedBuildings.allyBuildings
	...
	core.allyWellAttacker		= sortedBuildings.allyWellAttacker
	core.allyOtherBuildings		= sortedBuildings.allyOtherBuildings
	core.shops					= sortedBuildings.shops
	
	
	if bDebugEchos then
		if true then
			BotEcho("enemyBuildings:") core.printGetTypeNameTable(core.enemyBuildings)
			...
			BotEcho("enemyOtherBuildings:") core.printGetTypeNameTable(core.enemyOtherBuildings)
			BotEcho("allyBuildings:") core.printGetTypeNameTable(core.allyBuildings)
			...
			BotEcho("allyWellAttacker: "..tostring(core.allyWellAttacker))
			BotEcho("allyOtherBuildings:") core.printGetTypeNameTable(core.allyOtherBuildings)
...
function core.GetClosestAllyBuilding(position)
	local closestDistSq = 99999999
	local closestBuilding = nil
		
	for key, building in pairs(core.allyBuildings) do
...
function core.InventoryContains (inventory, val, ignoreRecipes)
	if ignoreRecipes == nil then
		ignoreRecipes = false
	end
	...
	for slot = 1, 12, 1 do
		...
		if curItem then
			...
			if curItem:GetName() == val and (not ignoreRecipes or not curItem:IsRecipe()) then
...
function core.SortBuildings(tBuildingList, tSortedTable)
	...
	tSortedTable.allyMainBaseStructure		= tSortedTable.allyMainBasStructure or nil
	...
	for key, building in pairs(tBuildingList) do
		...
		if building ~= nil and building:IsBuilding() then
			--BotEcho("  "..building:GetTypeName())
			local team = building:GetTeam()
			if team == allyTeam then
				local bPlaced = false
				if building:IsTower() then
					tinsert(tSortedTable.allyTowers, building)
					bPlaced = true
				end
				if building:IsRax() then
					tinsert(tSortedTable.allyRax, building)
					bPlaced = true
				end
				...
				if not bPlaced then
					tinsert(tSortedTable.allyOtherBuildings, building)
				end
				tinsert(tSortedTable.allyBuildings, building)
			elseif team == enemyTeam then
				local bPlaced = false
				if building:IsTower() then
					tinsert(tSortedTable.enemyTowers, building)
					bPlaced = true
				end
				if building:IsRax() then
					tinsert(tSortedTable.enemyRax, building)
					bPlaced = true
				end
				if building:IsBase() then
				...
				if not bPlaced then
					tinsert(tSortedTable.enemyOtherBuildings, building)
				end
				tinsert(tSortedTable.enemyBuildings, building)
			end
			
			if building:IsShop() then
				tinsert(tSortedTable.shops, building)
			end
		end
	end
...
function core.ValidateUnitReferences()
	...
	core.ValidateReferenceTable(core.enemyBuildings)
	core.ValidateReferenceTable(core.enemyTowers)
	core.ValidateReferenceTable(core.enemyRax)
	core.ValidateReferenceTable(core.enemyOtherBuildings)
	core.ValidateReferenceTable(core.allyBuildings)
	core.ValidateReferenceTable(core.allyTowers)
	core.ValidateReferenceTable(core.allyRax)
	core.ValidateReferenceTable(core.allyOtherBuildings)
	core.ValidateReferenceTable(core.shops)
end
...
function core.GetAttackSequenceProgress(unit)
	--[[
	|----------attackDuration-------|   1700 (except for a few heroes)
	|---attackTime--|               |   1000 (except for a few heroes)
	|-------|      (actionTime)     |   variable, usually 300-500
