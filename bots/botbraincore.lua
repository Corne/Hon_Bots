...
core.runesOfBlightDef = nil
core.healthPotDef = nil
core.tpStoneDef = nil
...
core.nbTutorialBehaviorResetTime = core.SToMS(core.MinToS(9))
...
function object:onthink(tGameVariables)
	...
	StartProfile('Updates')
		StartProfile('Update local units')
...
function core.BotBrainCoreInitialize(tGameVariables)
	...
	local def = HoN.GetItemDefinition("Item_RunesOfTheBlight")
	core.runesOfBlightDef = core.WrapInTable(def)
	core.runesOfBlightDef.duration = 16000
	core.runesOfBlightDef.expireTime = 0
	core.runesOfBlightDef.stateName = "State_RunesOfTheBlight"	
	
	def = HoN.GetItemDefinition("Item_HealthPotion")
	core.healthPotDef = core.WrapInTable(def)
	core.healthPotDef.duration = 10000
	core.healthPotDef.expireTime = 0
	core.healthPotDef.stateName = "State_HealthPotion"
	
	def = HoN.GetItemDefinition("Item_HomecomingStone")
	core.tpStoneDef = core.WrapInTable(def)
	core.tpStoneDef.channelTime = 3000
...
function core.UpdateLocalUnits(botBrain)
	...
	if bDebugEchos then
		BotEcho("LocalUnits:")
		Echo("Neutrals\n{")			core.printGetTypeNameTable(core.localUnits["Neutrals"])			Echo("}")
		Echo("AllyTowers\n{")		core.printGetTypeNameTable(core.localUnits["AllyTowers"])       Echo("}")
		Echo("AllyRax\n{")			core.printGetTypeNameTable(core.localUnits["AllyRax"])          Echo("}")
		Echo("AllyBuildings\n{")	core.printGetTypeNameTable(core.localUnits["AllyBuildings"])    Echo("}")
		Echo("AllyUnits\n{")		core.printGetTypeNameTable(core.localUnits["AllyUnits"])        Echo("}")
		Echo("AllyHeroes\n{")		core.printGetTypeNameTable(core.localUnits["AllyHeroes"])       Echo("}")
		Echo("AllyCreeps\n{")		core.printGetTypeNameTable(core.localUnits["AllyCreeps"])       Echo("}")
		Echo("Allies\n{")			core.printGetTypeNameTable(core.localUnits["Allies"])           Echo("}")
		Echo("EnemyTowers\n{")		core.printGetTypeNameTable(core.localUnits["EnemyTowers"])      Echo("}")
		Echo("EnemyRax\n{")			core.printGetTypeNameTable(core.localUnits["EnemyRax"])         Echo("}")
		Echo("EnemyBuildings\n{")	core.printGetTypeNameTable(core.localUnits["EnemyBuildings"])   Echo("}")
		Echo("EnemyUnits\n{")		core.printGetTypeNameTable(core.localUnits["EnemyUnits"])       Echo("}")
		Echo("EnemyHeroes\n{")		core.printGetTypeNameTable(core.localUnits["EnemyHeroes"])      Echo("}")
		Echo("EnemyCreeps\n{")		core.printGetTypeNameTable(core.localUnits["EnemyCreeps"])      Echo("}")
		Echo("Enemies\n{")			core.printGetTypeNameTable(core.localUnits["Enemies"])			Echo("}")		
	end
end
...
function core.UpdateLane()
	...
	if HoN.GetRemainingPreMatchTime() <= core.teamBotBrain.nInitialBotMove then 
		if core.tMyLane == nil or curTime > core.nLaneUpdateTime or core.nTeamBotBrainPushState ~= teamBotBrain.nPushState then
			core.tMyLane = core.teamBotBrain:GetDesiredLane(core.unitSelf.object or core.unitSelf)
...
function core.FindItems(botBrain)
	...
	core.nFindItemsTime = HoN.GetGameTime()
	return true
end
