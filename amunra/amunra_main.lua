...
local function HealAtWellUtilityOverride(botBrain)
	...
	if (not skills.abilRebirth:CanActivate()) or (object.enemyLastHPPerc - unitSelf:GetHealthPercent() <= 50) then
		object.HealAtWellUtilityOld(botBrain)
		--BotEcho("here you go "..object.enemyLastHPPerc - unitSelf:GetHealthPercent() )
	else 
		--BotEcho('WHATT?')
	end
end
object.HealAtWellUtilityOld = behaviorLib.HealAtWellUtility
behaviorLib.HealAtWellUtility = HealAtWellUtilityOverride
...
local function abilityPush(botBrain, unitSelf)
	...
	if  abilMeteor:CanActivate() and ShouldAttackHpBased(unitSelf, object.tAbilityValuesCreeps.nMeteorHpThreshold) then 
		...
		if  nDistanceSq < (nMaxRange * nMaxRange) and nDistanceSq > (nMinRange * nMinRange) then --range check for meteor push
			return activateMeteor(botBrain, vCreepCenter, debugAbilityPush)
		end
	end
...
---------------------------------------------------------------
--- Chat functions - noone likes predefined stuff... NOONE!  --
	...
	if HoN.GetMatchTime() > 0 then
		...
		if object.bUltWasUpLately then
			...
			if(nToSpamOrNotToSpam < core.nRespawnChatChance) then
				local nMessage = random(#object.respawnMessages) 
...
behaviorLib.MidItems = {"Item_Steamboots", "Item_Shield2", "Item_MagicArmor2", "Item_PlatedGreaves"} --Item_Shield2 is helm of the black legion || hotbl; Item_MagicArmor2 is shamans headress
